...
export function renderValuationCard(result: ValuationResult): string {
	...
	row(
		'标准',
		`${result.standard.name}${result.standard.version ? ` v${result.standard.version}` : ''}`,
	);
...
export function renderBatchTable(results: ValuationResult[]): string {
	...
	for (let i = 0; i < results.length; i++) {
		...
		const name = r.image.url.split(/[/\\]/).pop()?.slice(0, 20).padEnd(20) ?? 'unknown'.padEnd(20);
		...
		const status = (r.image.corruption === 'ok' ? 'OK' : r.image.corruption).padEnd(8);
...
export function renderSearchResults(records: ValuationRecord[]): string {
	...
	for (const r of records) {
		...
		lines.push(
			`    价值: ¥${r.minValue.toFixed(2)} - ¥${r.maxValue.toFixed(2)} (不确定性: ¥${r.uncertainty.toFixed(2)})`,
		);
