...
describe('storage-repository', () => {
	...
	it('searches by multiple formats', () => {
		valuationRepo.insert(makeInsert({ imageFormat: 'jpeg', description: 'jpg' }));
		valuationRepo.insert(makeInsert({ imageFormat: 'png', description: 'png' }));
		valuationRepo.insert(makeInsert({ imageFormat: 'webp', description: 'webp' }));

		const results = searchRepo.search({ format: ['jpeg', 'png'] });
		expect(results).toHaveLength(2);
		expect(results.map((r) => r.imageFormat).sort()).toEqual(['jpeg', 'png']);
	});
	...
	it('checks existence by hash and standard name', () => {
		valuationRepo.insert(makeInsert({ imageHash: 'hash1', standardName: 'photo' }));
		valuationRepo.insert(makeInsert({ imageHash: 'hash1', standardName: 'art' }));

		expect(valuationRepo.existsByHashAndStandard('hash1', 'photo')).toBe(true);
		expect(valuationRepo.existsByHashAndStandard('hash1', 'art')).toBe(true);
		expect(valuationRepo.existsByHashAndStandard('hash1', 'portrait')).toBe(false);
