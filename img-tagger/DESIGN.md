...
- 标签**先创建后使用**：LLM 打标时只能使用已存在的标签，或通过 `create_tag` 工具新建；`tag` 子命令与 `tags create` 走同样校验。
- 每个标签包含**标签描述**。
- 标签**不能包含空格**；另有全局字符规范与分类标准中的附加命名规范（见 §12）。
- 图片的全部标签构成**标签串**（tag string，定义见 §1.3、§11.1）。
- **冲突**：数据库中标签串相同（非空）的不同图片互为冲突。
...
| 术语 | 简略定义 | 出处 |
|------|----------|------|
| **标签串**（`tag_string`） | 图片全部标签名按 UTF-8 逐字节升序用单个空格连接所得字符串；空标签集为 `''`。`tag_string` 缓存列、冲突判定与查询均以它为准 | §11.1 |
| **冲突** | 两条不同 `image` 记录（`image_hash` 不同）的 `tag_string` 非空且相等；`tag_string = ''` 的图片除外 | §11.2 |
| **未完成打标** | `tag_string = ''` 的图片：视为尚未打标，`--mode skip` 重跑会重新打标，`images prune` 可清除 | §11.2 |
| **打标路径** | 调 LLM 的打标流程（顶层默认命令），冲突进入冲突解决协议自动处理 | §8 |
| **手动路径** | 不调 LLM 的命令（`tag`/`tags` 等），冲突直接报错、不进入解决协议 | |
| **先创建后使用** | 打标仅能使用已存在标签，或经 `tags create`/`create_tag` 新建；手动与 LLM 路径走同一校验 | §12.3 |
| **无冲突不变式** | 库中任意时刻不存在 `tag_string` 相同的两条不同图片（由写前冲突检测 + 回滚保证） | §13 |
| **存储序**（字节序） | `tag_string` 缓存、冲突判定、SQL 查询所采用的 UTF-8 字节升序；大小写敏感、跨平台确定 | §11.1 |
| **展示序**（locale 序） | 命令表格、错误消息、冲突反馈等人类可读输出的排序（`Intl.Collator`：数字自然序、大小写不敏感、中文拼音序）；不进入 JSON 输出 | §11.1 |
| **全局硬性规范** | 标签名的程序校验硬约束（非空、长度 1–64、禁空白、禁保留字符等），违反即报错/反馈重试 | §12.1 |
| **附加命名规范** | 分类标准为标签名附加的自然语言软约束与可选 `tagPattern` 正则（可程序校验） | §12.2 |
| **上下文图片集合** | 当前打标流程可触达的图片集合（当前打标图 + 冲突反馈附带的冲突图）；`get_exif`/`modify_image_tags` 仅允许操作集合内图片 | §8.3 |
...
**所有参数均经预处理语句绑定**。结果按 `tagged_at` 倒序。
...
- `failLogDir`（默认空串 = 禁用）为失败日志目录；**若设置**（非空），每张失败图写入一个完整请求 JSON 文件（沿用 img-val `IMGVAL_FAIL_LOG` 行为），空串/缺省则禁用。
...
- 文件内容保留**完整轨迹**供离线回放/调试：原始 prompt、消息历史（含工具调用与结果，**其中图片块以占位符替代、不含任何像素数据**）、工具调用序列、失败错误码与消息、失败阶段标记、图片路径与 hash、EXIF（若 `failLogIncludeExif` 启用）。
...
| `failLogDir` | string, `""` | `IMGTAGGER_FAIL_LOG` | 失败日志目录；省略或空串视为禁用（TOML 1.0 无 null，以省略/空串表达），仅非空启用（§4.3） |
...
| 工具 | 参数 | 说明 |
|------|------|------|
| `create_tag` | `name`、`description` | 新增标签；执行校验（含 `tagPattern`）；重复名返回已存在标签 `{ id, name, description }`（含既有描述供 LLM 判断是否真正复用，不报错） |
...
| `modify_image_tags` | `image`（路径或 hash）、`tags: string[]` | 修改**已有图片**的标签集合（整体替换），用于解决标签串冲突（见 §8.3）；`image` **必须为上下文图片集合**中的图片，集合外 → 报错 `MODIFY_PATH_NOT_ALLOWED`（越权保护）；**`tags` 必须已存在**（先创建后使用）——含不存在标签 → 报错并列出缺失清单，提示先用 `create_tag` 新建（新建职责归 `create_tag`，本工具不代建）；执行前**默认需用户批准**，拒绝时把拒绝原因反馈给 LLM。本工具不创建标签，故**不计入** `createTagPerCycle`（§8.3.7） |
| `submit_tags` | `tags: string[]`、`description?` | 提交最终标签结果（Anthropic 约束解码用，参照 img-val `submit_valuation`）；`description` **不入库**、不影响打标结果。Schema 由 `--verbose` 决定：verbose 时字段为 `tags` + `description`（必填），非 verbose 时 schema 仅含 `tags`，`description` 完全不出现在系统提示/工具定义/结果 schema 中（provider 层按需重组，§2）。 |

**两家 provider 统一走工具调用**：`submit_tags` 作为工具定义提供给 LLM，最终结果由工具参数提取；provider 层**不启用 `response_format`**——OpenAI API 不允许同一请求同时携带 `tools` 与 `response_format=json_schema`，而打标全程必须携带工具，故 `response_format` 结构化输出不可用，OpenAI 与 Anthropic 收敛为同一路径。`--verbose` 时字段为 `tags` + `description`，否则仅 `tags`。OpenAI 侧如需严格结构约束，在工具 `parameters` 上加 `strict: true`（shared provider 层适配，见 §2）。
...
4. 校验 `submit_tags` 结果：
   ...
   - 标签名逐个执行全局硬校验与标准 `tagPattern`，失败同样反馈重试。
   - 标签数量超上限（默认 50 或标准 `maxTags`）→ 反馈 LLM 重试，并提示缩减到上限内。
5. **流级标签清理（flow-scoped tag cleanup）**：流程自进入工具循环起记录本次经 `create_tag` **成功新建**的标签集合；流程结束时（成功提交、任一上限耗尽中止、修复状态退出）执行清理——从该集合中删除**当前无任何 `image_tag` 引用**的标签：成功路径回收「建而未用」（如新建 3 只用 2）的标签，失败/中止路径回收未随提交入库的全部新建标签，保证失败/中止不向全局标签库残留孤儿（与 §4.1「该图不写库」语义一致）。删除前做引用检查，仅零引用时删除，避免误删其它流程已引用的标签；引用检查与删除在同一同步临界区（§13）内完成，与其它写操作互斥。若另一流程的 `submit_tags` 恰在清理之后引用该标签，其会在自身临界区重新做存在性校验并获「不存在」反馈、可经 `create_tag` 重建，无一致性风险。清理不影响已成功提交图片的标签（其标签必被引用，自然保留）。该清理与 §8.3.7 的 `createTagPerCycle` 计数相互独立。

### 8.3 冲突解决协议（LLM 自动解决）

**冲突解决周期（conflict resolution cycle）**：自首次产生冲突的 `submit_tags` 起，至成功提交或任一额度耗尽中止该图打标止。周期内 §8.3.6 两类重试上限与 §8.3.7 `create_tag` 新建数上限独立计数；周期终结后（若后续提交再冲突）进入新周期、重新计数。**冲突期间新增的工具轮次（每次冲突反馈后 LLM 的再调用）同样计入全局 `maxToolRounds`**；§8.3.6 两类额度与 §8.3.7 的 `createTagPerCycle` 是在此之上的**专项上限**，任一先耗尽即中止该图打标——总轮次以 `maxToolRounds` 封顶，专项额度不增加全局轮次。

1. 校验通过后计算 `tag_string`，基于当前库执行冲突检测。**上下文图片集合**在进入工具循环前即含目标图（`get_exif` 可查询自身）；集合仅追加不清除（见步骤 2）。
2. 冲突时构建反馈消息：冲突 `tag_string`、冲突图片**内容**（`processImage` 缩放后随消息发送）及路径，提示"必须解决冲突后方可提交，不得与现有图片标签串相同"。该图随即**追加**入上下文图片集合，此后 `get_exif` 与 `modify_image_tags` 可对其操作。正常路径下该集合至多累计 **1 + 冲突次数** 张图片（每轮至多 1 张冲突图；冲突次数受 §8.3.6 上限约束）。集合生命周期与单张图片的整个打标流程一致：流程开始创建、成功提交或中止时释放，跨冲突周期不清除、保证 LLM 对历史冲突图上下文的连续性；上界即 `1 + 冲突次数`，由 §8.3.6 重试额度确定，无无限增长路径。
   **冲突数量**：库保持无冲突不变式，故每次提交最多与 **1** 张图冲突，发送该图内容即可。若一次提交与多张图冲突（>1），属**不可达分支**（不变式已被破坏，多为历史数据问题）：立即中止该图打标并进入**修复状态**——**向 stderr 输出告警与诊断日志**「数据库处于异常状态：存在标签串相同的多张图片」，附冲突涉及的图片明细（`path`/`image_hash`/`tag_string`）供定位排查；**不向 LLM 发送冲突图片、不做自动解决**，并**重置全部重试计数器**（`tagConflictRetries`/`modifyRejectLimit`/`createTagPerCycle`）；提示先运行 `imgtagger check` 定位，再以 `images prune`、`tag set` 或 `tags delete` 人工修复。批量模式下该图计入失败汇总、继续处理其它图；该状态非 LLM 打标失败，**仅通过 stderr 告警、不写入 failLogDir**（§4.3）；该图期间经 `create_tag` 新建的标签由 §8.2 流级清理回收。
...
4. **`modify_image_tags` 审批**：工具执行前弹出交互确认（显示目标图片路径、原标签串、新标签串，`y/N`）。默认拒绝；**用户拒绝时不执行修改**，并把拒绝原因构造为工具执行结果反馈给 LLM（"用户拒绝了该修改：<原因>"），LLM 可换用 `create_tag` 方案或提出新的修改方案。非 TTY 环境（无交互）视为默认拒绝；**`--yes` 模式下跳过确认直接执行**（即授权自动修改他人图片标签）。审批不消耗 LLM 工具轮次，但**用户拒绝次数计入独立的 `modifyRejectLimit` 上限**（见下条），与 LLM 工具反馈重试额度互不影响。**获批即持久化**：获批的 `modify_image_tags` 修改真实提交，不随本图后续失败（轮次/额度耗尽、修复状态）回滚；审批即对持久化变更的授权，确认文案应提示目标图片将被实际修改。**与 `images prune` 非 TTY 直接报错退出不同**：此处非 TTY 默认拒绝后**继续流程**（把拒绝反馈给 LLM 换方案）——差异有意为之：prune 为破坏性命令、无法确认即中止；打标审批拒绝只是「不执行该工具」，流程仍可经 `create_tag` 等路径继续。
5. 工具执行校验：`modify_image_tags` 先校验 `tags` 均已存在（含不存在标签 → 报错并反馈缺失清单），通过后重算目标图片 `tag_string`，若与其它图片也冲突则同样构造反馈要求继续解决；对目标图的重算与写入先获取当前图 + 目标图两把 per-hash 锁（按 hash 升序）。`create_tag`/`submit_tags` 路径的校验同单轮打标流程。
6. **两类重试上限**，各自计数、互不影响：
   - **LLM 工具反馈重试**（`tagConflictRetries`，默认 2，见 §5.2）：一次**冲突解决周期**（从首次提交产生冲突起，至成功提交或达上限止）内，`create_tag`、`submit_tags` 反馈失败（含校验失败、不存在标签、数量超限、`tagPattern` 不匹配）共享该额度，**累计**计数；累加达上限 → 中止该图打标。
...
- **打标必须调用 LLM 且启用工具调用**（无禁用选项）。
- 图片送入前经 shared `processImage` 缩放（`maxImageDimension`，默认 1568，见 §5.2）。
...
- `tags create` 的 `--standard <name|path>`：决定校验所依据的分类标准，**缺省为内置默认标准**（`default`），与 `tag set/add`（§9.2）一致；标准含 `tagPattern` 时追加正则校验（§12.3 第 3 步），不含 `tagPattern` 时等价于仅全局硬规范校验。`--standard` 缺省时走默认标准，保证 `tagPattern` 校验在手动路径也生效。
...
沿用 img-val 模式：YAML frontmatter + Markdown 正文（gray-matter 解析 + zod 校验），放在 `standardsDir`（默认 `~/.img-data/tag-spec/*.md`，见 §5.2）。
...
tagPattern: ^nature-     # 可选：标签名正则（可执行附加规范；`^...$` 锚定）
maxTags: 30              # 可选：单图标签数硬上限（默认 50）
---
```

> **注**：`tags` 字段仅用于用户在本地按类别组织标准文件，**不会出现在发送给 LLM 的 prompt/工具描述中**；标签库为全局且不与标准关联，故 `tags list` 不提供按标准过滤（见 §9.4）。
...
- **存储/规范化排序**（决定 `tag_string` 与冲突判定）：按标签名的 UTF-8 字节序列逐字节比较（`Buffer.compare` 语义），大小写敏感；对良构 UTF-8，该序**等价于按 Unicode 码点排序**，与之真正不同的是 **locale 排序**（`Intl.Collator`：大小写折叠、重音折叠、中文按拼音、数字自然序等，见下）。字节序的直观后果是 **ASCII 大写 < ASCII 小写 < 多字节（中文）**，例如 `'Apple' < 'apple' < '中'`——该顺序与人类直觉/locale 排序不同，但保证跨平台确定性。**存储不变**：`tag_string` 缓存、冲突判定、SQL 查询均以该序为准。
...
> **设计权衡**：存储采用 UTF-8 字节序是显式取舍——换取跨平台确定性、无 locale 依赖、可直接用 SQLite 字节比较排序；代价是存储序与人类直觉/locale 排序不一致，中文标签混合时 `tag_string` 观感无序。展示层用 `Intl.Collator` 缓解；**存储序**（`tag_string` 缓存、冲突判定、SQL 查询）与**展示序**（表格/消息/反馈）严格分离，JSON 输出只含存储序，避免双出口歧义。
...
标签名必须同时满足（总则：`-` 允许但**非首字符**；查询中叶子开头的 `-` 仅作反向操作符，见 §3.1 词法）：

1. 非空，长度 1–64。
2. 不能包含空白字符（空格、制表符等）——标签串以空格连接的前提。
3. 不能包含查询语法保留字符：`|` `(` `)` `*`（与 §3.1 标签名词法共用同一保留字符集，保证任何合法标签名均可无歧义构造查询）。
4. 必须以字母（Unicode）、数字或 `_` 开头——蕴含"不能以 `-` 开头"。
5. 仅允许 Unicode 字母、数字、`_`、`-`、中文等（即不包含第 3 条保留字符与空白即可，宽松默认，允许中文）。
6. 单一图片标签数上限：默认 **50**；可由分类标准 frontmatter `maxTags` 覆盖（见 §10、§8.2）。

标签名可含 `%`、`_`（不禁止）；查询时普通项按精确匹配、通配项按转义后 LIKE 处理（见 §3.4），故无歧义。

### 12.2 分类标准中的附加命名规范（软约束 + 可选可执行）

分类标准文件正文中以自然语言描述附加命名规范（如"标签必须含主题前缀 `nature-`"、"长度不超过 12"、"必须为中文"）；亦可选用 frontmatter 字段 `tagPattern`（正则）将其程序化校验。该规范：

- 自然语言部分：作为系统提示（覆盖全部决策点，含 `submit_tags`）与 `create_tag` 工具描述的一部分提供给 LLM，由 LLM 自觉遵守，不程序校验；
- `tagPattern` 部分：在全局硬性规范之后**追加程序校验**，**手动与 LLM 路径一律强制**；正则不匹配 → 报错（手动路径）或反馈 LLM 重试（打标路径）；
- 全局硬性规范始终叠加生效。
...
4. **软约束提示（仅 LLM 路径）**：无 `tagPattern` 时，自然语言附加规范随系统提示与 `create_tag` 工具描述提供给 LLM，由 LLM 自觉遵守，**不程序校验、不构成失败环节**；手动路径至第 3 步即止。
...
- **无冲突不变式**：由于所有写操作提交前均执行冲突检测并在冲突时回滚，数据库在任何时刻保持"无冲突"不变式；若某次提交出现与多于一张图冲突的状态，即标志着不变量被破坏的异常（见 §11.3、§8.3.2）。**启动时信任历史库保持该不变式，不做启动期全量校验**（需要时用 `imgtagger check` 手动校验）；运行期若出现多图冲突即判定数据库处于异常状态，进入**修复状态**、中止打标，用户应先运行 `imgtagger check` 定位异常。
